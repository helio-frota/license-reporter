...
const dual = require('../../src/utils/dual');
...
const htmlReport = async (reportData) => {
  try {
    const output = await html.parse(reportData);
...
const transform = (data, canonicalNameMapper, cwd, ilc, fdt, s, outputDir) => {
  ldir = join(process.cwd(), outputDir);
